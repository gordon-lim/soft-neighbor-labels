...
class NoisyCIFAR10(datasets.CIFAR10):
    ...
    def __init__(
        ...
        if noisy_label_file:
            ...
            noise_cnt = sum(np.array(self.noisy_labels) != np.array(self.given_labels))
            self.noise_rate = noise_cnt / len(self.given_labels)
        else:
            self.targets = self.given_labels.copy()
            self.noise_rate = 0.0

        self.noise_or_not = np.array(self.targets) == np.array(self.given_labels)
