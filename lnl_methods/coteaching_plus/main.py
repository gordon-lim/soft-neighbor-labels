...
if args.dataset=='cifar10':
    ...
    train_dataset = NoisyCIFAR10(root='../data/',
                                ...
                                noisy_label_file=args.noisy_label_file
                                )
    
    test_dataset = NoisyCIFAR10(root='../data/',  
                                train=False, 
                                transform=test_transform,
                                noisy_label_file=None # Clean
                                )
...
if args.dataset=='cifar100':
    ...
    train_dataset = NoisyCIFAR100(root='../data/',
                                train=True, 
                                transform=train_transform,
                                noisy_label_file=args.noisy_label_file
                                )
    
    test_dataset = NoisyCIFAR100(root='../data/',  
                                train=False, 
                                transform=test_transform,
                                noisy_label_file=None # Clean
                                )
...
if args.dataset == 'imagenet_tiny':
    ...
    train_kv = "train_noisy_%s_%s_kv_list.txt" % ('', 0.0) 
...
if args.dataset == 'imagenet_tiny':
    noise_or_not = np.load(os.path.join(data_root, 'noise_or_not_%s_%s.npy' %('', 0.0)))
else:
...
model_str = args.dataset + '_%s_' % args.model_type + os.path.basename(args.noisy_label_file).replace('.csv', '')
...
def train(train_loader,epoch, model1, optimizer1, model2, optimizer2):
    ...
    for i, (indexes, data, labels, _) in enumerate(train_loader):
...
def evaluate(test_loader, model1, model2):
    ...
    correct1 = 0
    total1 = 0
    for _, data, labels, _ in test_loader:
        if args.dataset=='news':
            data = Variable(data.long()).cuda()
        else:
            data = Variable(data).cuda()
...
def main():
    ...
    wandb.init(project="coteaching_plus", name=run_name, config=args)  # Initialize WandB with your project name
    ...
    for epoch in range(1, args.n_epoch):
        ...
        wandb.log({
            "epoch": epoch,
            "train_accuracy_model1": train_acc1,
            "train_accuracy_model2": train_acc2,
            "test_accuracy_model1": test_acc1,
            "test_accuracy_model2": test_acc2,
        })
